...
export class ScheduleService {
    ...
    async create(createScheduleDto: CreateScheduleDto): Promise<Schedule> {
        ...
        if (!section) {
            throw new NotFoundException(`Section with ID ${createScheduleDto.sectionId} not found`);
        }
    ...
    async getScheduleWithDocument(scheduleId: string): Promise<Schedule> {
        ...
        try {
            ...
            if (!rawResult || rawResult.length === 0) {
                throw new NotFoundException(`Schedule document not found for ID: ${scheduleId}`);
            }
            
            const scheduleDoc = rawResult[0];
            
            console.log(`Document found: ${scheduleDoc.documentName}, Size: ${scheduleDoc.documentData?.length || 0} bytes`);
