...
export class ProfileRequestsService {
  ...
  async create(createProfileRequestDto: CreateProfileRequestDto) {
    ...
    const newRequest = this.profileRequestRepo.create(createProfileRequestDto);
  ...
  async update(id: string, updateProfileRequestDto: UpdateProfileRequestDto & { processedById?: string }) {
  ...
  private async applyProfileChanges(request: ProfileRequest) {
    ...
    if (request.numeroTelephonePrincipal !== undefined) {
      updateData['numeroTelephonePrincipal'] = request.numeroTelephonePrincipal;
    }
    
    if (request.numeroTelephoneSecondaire !== undefined) {
      updateData['numeroTelephoneSecondaire'] = request.numeroTelephoneSecondaire;
    }
    
    if (request.adressePostale !== undefined) {
      updateData['adressePostale'] = request.adressePostale;
    }
    
    if (request.codePostal !== undefined) {
      updateData['codePostal'] = request.codePostal;
    }
    ...
    if (request.contactEnCasDurgence !== undefined) {
      updateData['contactEnCasDurgence'] = request.contactEnCasDurgence;
    }
