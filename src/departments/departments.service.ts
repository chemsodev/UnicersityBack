...
export class DepartmentService {
    ...
    async getDepartmentSections(id: string): Promise<Section[]> {
        const department = await this.departmentRepository.findOne({
            where: { id },
            relations: ['sections'],
        });
    ...
    async addSectionToDepartment(departmentId: string, sectionId: string): Promise<Department> {
        const department = await this.departmentRepository.findOne({
    ...
    async removeSectionFromDepartment(departmentId: string, sectionId: string): Promise<Department> {
        const department = await this.departmentRepository.findOne({
            where: { id: departmentId },
            relations: ['sections'],
        });
        ...
        department.sections = department.sections.filter(section => section.id !== sectionId);
        return this.departmentRepository.save(department);
    }
}