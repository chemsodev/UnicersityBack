  ...
  canActivate(context: ExecutionContext): boolean {
    ...
    if (!userRole && !userType) {
      ...
      if (requiredRoles.includes(AdminRole.ENSEIGNANT) && 
          (request.path.includes('/enseignants/') || request.path.includes('/api/enseignants/'))) {
      ...
      if (requiredRoles.includes(AdminRole.ETUDIANT) && 
          (request.path.includes('/etudiants/') || request.path.includes('/api/etudiants/'))) {
    ...
    if (requiredRoles.includes(AdminRole.ETUDIANT)) {
      ...
      if (isStudent) {
        ...
        this.logger.log(`Student access granted. UserId: ${user.userId}, Path: ${request.path}`);
        
        // If this is a specific student profile request, check if IDs match
        if (request.path.match(/\/etudiants\/\d+/) || request.path.match(/\/api\/etudiants\/\d+/)) {
          const requestedId = request.params.id || request.path.split('/').pop();
          this.logger.log(`Student accessing profile: ${requestedId}, Student's own ID: ${user.userId}`);
    ...
    const hasRequiredRole = requiredRoles.some(role => {
      ...
      // Check against string version of role
      if (userRole === role.toString()) return true;
